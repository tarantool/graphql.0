...
local RESULTING_OBJECT_CNT_MAX = 10000
local FETCHED_OBJECT_CNT_MAX = 10000
...
local function process_tuple(state, tuple, opts)
    ...
    assert(qstats.fetched_object_cnt <= fetched_object_cnt_max,
            ('fetched object count[%d] exceeds limit[%s] '..
                    '(set `fetched_object_cnt_max` in accessor)'):format(
    ...
    assert(qstats.resulting_object_cnt <= resulting_object_cnt_max,
            ('returning object count[%d] exceeds limit[%s] '..
                    '(set `resulting_object_cnt_max` in accessor)'):format(
...
--- `fetched_object_cnt_max` are optional natural numbers whish help to control
